...
const expandSection = (sectionName) => {
  ...
  if (root.getAttribute('data-expanded') === null) {
    ...
    content.style.animation = 'slideDownEnhanced 0.5s cubic-bezier(0.4, 0, 0.2, 1)';
  } else {
    // Hide section
...
const colourInput = async (inputNode, allowedToBeEmpty, allowedToBeFile, allowedToBeADirectory) => {
  ...
  if (
    ...
    inputNode.classList.remove('animate-error');
    inputNode.classList.add('animate-success');
    if (window.soundSystem) {
      window.soundSystem.play('success');
    }
    setTimeout(() => inputNode.classList.remove('animate-success'), 600);
    return true;
  } else {
    inputNode.style.borderColor = 'var(--error)';
    inputNode.style.boxShadow = '0 0 0 3px var(--error-transparent)';
    inputNode.classList.remove('animate-success');
    inputNode.classList.add('animate-error');
    if (window.soundSystem) {
      window.soundSystem.play('error');
    }
    setTimeout(() => inputNode.classList.remove('animate-error'), 600);
    return false;
  }
};
